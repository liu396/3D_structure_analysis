...
//    exit(0);
	...
	interval=(up_limit-low_limit)/double(bin);
	...
	for(i=0;i<SurfaceAtomPosition.size();i++){
		...
		distance=diameter(SurfaceAtomPosition[i],rc_body/2.0);
		...
		m=int((k_1-low_limit)/interval);
		if (m>=bin) m=bin-1;
		if (m<0) m=0;
		n=int((k_2-low_limit)/interval);
		if (n>=bin) n=bin-1;
		if (n<0) n=0;
		cur_data[m*bin+n]++;
	...
	for(i=0;i<bin;i++){
		for(j=0;j<bin;j++){
			cur_output<<i<<"   "<<j<<"   "<<cur_data[i*bin+j]<<'\n';
		}
	}
